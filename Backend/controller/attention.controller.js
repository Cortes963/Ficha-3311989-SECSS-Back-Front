...
export async function indexReport(req,res){try
    {const {pagina,limite,offset}=page(req.query);

    const [[c]]=await db.query ('SELECT COUNT(*) total FROM reporte');

    const [datos]=await db.query (`SELECT r.*,CONCAT(u.primer_nombre,' ',u.primer_apellido) 
        celador FROM reporte r JOIN usuario u ON u.id=r.id_usuario_celador
         ORDER BY r.fecha_hora DESC LIMIT ? OFFSET ?`,
    [limite,offset]);res.json({
        ok:true,pagina,limite,total:c.total,datos
    }
);
}
catch(e){
...
export async function indexPqrs(req,res){try
    {const {pagina,limite,offset}=page(req.query);
    const own=req.user.roles.includes('ADMINISTRADOR')?'':'WHERE p.id_usuario=?';

    const args=own?[]:[actorId(req)];const [[c]]=await db.query(`SELECT COUNT(*) total FROM 
        pqrs p ${own}`,args);

    const [datos]=await db.query(`SELECT p.*,EXISTS(SELECT 1 FROM respuesta r WHERE r.id_pqrs=p.id) 
        tiene_respuesta FROM pqrs p ${own} ORDER BY p.fecha_creacion DESC LIMIT ? OFFSET ?`,
...
export async function storePqrs(req,res){try{required(req.body,['asunto','cuerpo']
...
export async function storePqrsAnswer(req,res){
    ...
    const c=await db.getConnection();try{required(req.body, ['asunto','cuerpo']
    ...
    if(!p.length)throw Object.assign(new Error('PQRS no encontrada.'),
    {status:404});
    ...
    await c.query ('INSERT INTO respuesta (id_pqrs,id_usuario_administrador,asunto,cuerpo)  VALUES (?,?,?,?)',
        [id,actorId(req),req.body.asunto,req.body.cuerpo]);
        await c.query('UPDATE pqrs SET estado=? WHERE id=?',
            [PQRS.RESUELTO,id] );
            await c.commit();res.status(201).json({
                ok:true,mensaje:'Respuesta registrada.'  });
        }
...
export async function updatePqrsState(req,res){try
    {
    const id=integer(req.params.id,'id'), estado=Number(req.body.estado);
    if(!Object.values(PQRS).includes(estado))throw Object.assign(new Error('Estado PQRS inválido.'),
    {status:400});
    
    const [r]=await db.query('UPDATE pqrs SET estado=? WHERE id=?',
     [estado,id]);if(!r.affectedRows)
        return res.status(404).json({
            ok:false,mensaje:'PQRS no encontrada.' });
...
export async function indexAnswer(req,res){
    try{const [datos]=await db.query
    ('SELECT * FROM respuesta ORDER BY id DESC');
...
export async function storeReport(req, res) {
  try {
    ...
    return res.status(201).json({
      ok: true, mensaje: 'Reporte registrado correctamente',
  ...
  try {
    ...
    const [filas] = await pool.query(
      `SELECT
         ...
         CONCAT(u.primer_nombre, ' ', u.primer_apellido) AS celador FROM reporte r  INNER JOIN usuario 
         u ON u.id = r.id_usuario_celador  WHERE r.id = ?`,
      [id]
    );
