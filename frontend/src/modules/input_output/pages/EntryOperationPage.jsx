...
export const EntryOperationPage = () => {
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((row) =>
      [row.usuario, row.numero_documento, row.tipo_vehiculo, row.identificador_vehiculo]
  ...
  const selectQuota = async (row) => {
    setSelected(row);
    setQuery(`${row.usuario || ''} · ${row.identificador_vehiculo || ''}`);
    setDetail(null);
  ...
  return (
    <section className="card p-4">
      <h2 className="h4">Registrar operación</h2>
      ...
      <FormField
        ...
        placeholder="Seleccione un cupo"
      />
      ...
      <div className="list-group mb-3">
        {filteredRows.map((row) => (
          <button
            ...
            key={`${row.id_usuario}-${row.id_vehiculo}`}
            onClick={() => selectQuota(row)}
          >
            {row.usuario || row.numero_documento} · {row.tipo_vehiculo} · {row.identificador_vehiculo}
          </button>
        ))}
        {filteredRows.length === 0 && (
          <div className="list-group-item text-muted">No hay coincidencias.</div>
        )}
      </div>

      {loadingDetail && <div className="alert alert-info">Cargando autorización, aprendiz, centro y vehículo...</div>}
      ...
      <div className="d-flex gap-2">
        {detail && !loadingDetail && detail.estado === 1 && (
          <>
            <button type="button" className="btn btn-primary" onClick={() => operate('entrada')}>Registrar entrada</button>
            <button type="button" className="btn btn-outline-primary" onClick={() => operate('salida')}>Registrar salida</button>
          </>
        )}
      </div>
